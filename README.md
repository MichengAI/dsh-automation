...
- Official DSH peerDependencies are exactly `0.1.0-rc.8 || 0.1.1-rc.2 || 0.1.2-rc.1 || 0.1.5-rc.1 || 0.1.5-rc.2 || 0.1.6-alpha.1 || 0.1.6-alpha.2 || 0.1.7-alpha.1 || 0.1.7-alpha.2 || 0.1.7-rc.1`. `@deepseek-ai/dsh-agent-presets` is optional and stops at `0.1.6-alpha.2`, because `0.1.7` renamed that package to `@deepseek-ai/dsh-agent-preset-registry` while keeping the `agentPresets` service. Development dependencies are pinned to `0.1.7-rc.1`. Use one consistent official package version within each Host.
