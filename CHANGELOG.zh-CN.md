...
- 在 DSH 0.1.7 上，定时会话行使用宿主尺寸：5px 滚动条、10px 说明色时间、10px 操作间距、已归档用说明色、工作区树每层缩进 12px，悬停长标题会滚到末尾。更早宿主仍是 8px 滚动条、12px 时间和 12px 操作间距。相对时间补上月和年。仍然不提供置顶和手工排序。
- 在 DSH 0.1.6-alpha.2 上，定时会话行用这一版宿主的列表：工作区树每层缩进 12px，悬停时长标题一次滚到末尾，悬停卡片等 500ms。0.1.7 仍是匀速滚动和 800ms。0.1.6-alpha.1 及更早版本保持 16px 缩进和省略号。
