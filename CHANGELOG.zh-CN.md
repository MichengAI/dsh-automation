...
- 支持 DeepSeek Harness `0.1.7-rc.2`。`0.1.7-rc.1`、`0.1.5-rc.2`、`0.1.5-rc.1`、`0.1.2-rc.1`、`0.1.1-rc.2` 和 `0.1.0-rc.8` 仍可使用。
- 插件行为无需改动：rc.2 删除或改名的宿主导出与接口，本插件都没有使用。
- `0.1.7` 以 V4 存储会话，降级宿主后无法打开；升级后请重启 DSH 并刷新页面。
...
- 支持 DeepSeek Harness 0.1.7-rc.1。0.1.0-rc.8、0.1.1-rc.2、0.1.2-rc.1、0.1.5-rc.1 和 0.1.5-rc.2 仍可使用。
- 宿主如果还是 0.1.6，请先升级到 0.1.7-rc.1，再安装这一版。
- 0.1.7 的会话是 V4。宿主降级后，这些会话打不开。升级后请重启 DSH 并刷新页面。

## 0.1.47 - 2026-09-23

- 定时会话行一律使用 0.1.7 会话列表：5px 滚动条、10px 说明色时间、10px 操作间距、工作区树每层缩进 12px、已归档用说明色、长标题悬停匀速滚动、悬停卡片等 800ms，并提供工作区树、归档筛选和双击重命名。更早宿主的菜单仍走它们自己的 items 接口。相对时间包含月和年。仍然不提供置顶和手工排序。
- 升级后请重启 DSH 并刷新页面。

## 0.1.46 - 2026-09-23

- 支持 DeepSeek Harness 0.1.7，同时兼容更早版本。0.1.7 上定时侧栏会正常显示。
- 定时会话列表对齐官方任务侧栏：可按工作区树查看、筛选已归档，悬停可看到完成、待处理、子代理和归档。列表不提供置顶。
- 安装了归档插件后，会话菜单可以删除。停止并归档、撤销仍走宿主操作。
- 设置页、新建和删除任务、排序和总览开关换成统一界面。按 Esc 只关当前弹窗，不会连设置页一起关掉。
- 检查更新改成和技能管理一样的对话框。网页端加载的客户端包更小。
...
- 没有协作页签注册表时，自绘定时列表也会转发宿主的重命名/归档/删除/派生，菜单不再缺项。
...
- 在 DSH `0.1.6+` 宿主上，运行摘要优先订阅 `session/event` 增量；增量缺失或不完整时回退已弃用的同步快照。会话归属优先读 `deriveMessages`，旧宿主继续读事件。
- 总览卡片打开任务设置时兼容 Codex UI 设置页，不再只找官方设置弹窗。

## 0.1.42 - 2026-09-14

- 修复旧侧栏页签卸载时，可能误删同 ID 替代页签的问题。

## 0.1.41 - 2026-09-14

- 修复与归档、IM 插件组合使用时，侧栏定时入口及执行记录入口可能消失的问题。

## 0.1.40 - 2026-09-12

- 支持为单个自动化设置并发数量，让同一任务的多次执行可以同时进行。默认 `1`，旧任务保持原有行为；满额时跳过定时触发并拒绝手动启动，调低上限不取消已有运行。
- 间隔计划的最短间隔由 5 分钟缩短为 1 分钟。
- Agent 创建、更新工具支持 `max_concurrent_runs`，Web RPC 支持任务级 `maxConcurrentRuns`；它们与已停用的插件级全局并发配置无关。

## 0.1.39 - 2026-09-12

- 取消不同自动化任务的全局并发上限，同一目录下的不同任务也可同时运行，无需等待原有的两个运行名额。
- 旧配置 `maxConcurrentRuns` 不再生效。同一自动化仍保留重复运行保护：已有排队或运行记录时，不能再次手动启动，重叠的定时触发仍会跳过。
...
- 此版本未发布到 npm，请安装 `0.1.38` 或更高版本。
- 支持 DSH `0.1.5-rc.2`，继续兼容 `0.1.0-rc.8`、`0.1.1-rc.2`、`0.1.2-rc.1` 和 `0.1.5-rc.1`。

## 0.1.36 - 2026-09-10

- 适配 DSH `0.1.5-rc.1` 的显式 Agent 创建回调和 Web RPC 依赖注入，保留旧版接口；修正历史记录的手动/定时触发文案，并补充官方补丁引擎、真实 Connection、AgentLoop 与权限回归。
- 兼容新旧持久化会话列表；缺少枚举能力或发现异常枚举时保留全部会话关联，避免将仍存在的冷会话误判为已删除。
- 开发依赖锁定 `0.1.5-rc.1`，官方 Host peer 范围精确限定为 `0.1.0-rc.8 || 0.1.1-rc.2 || 0.1.2-rc.1 || 0.1.5-rc.1`。其他版本不在声明范围内，严格 peer 安装将失败；新增版本需扩展并运行独立的 `pnpm test:matrix`。

## 0.1.35 - 2026-09-09

- 调整模型菜单的宽度及辅助文本容器，精简模型条目显示。

- 统一设置页标题、说明与操作布局，标题及版本不再被维护按钮挤压；适配原生 DSH 设置内容区，无需安装 Codex UI。

## 0.1.34 - 2026-09-07

- 将发布工作流的 pnpm 版本与包内声明对齐，使标签发布能够进入完整校验。
...
- 移除无人值守自动化的固定工具白名单和后台 shell 禁令，工具调用及后台进程管理统一遵循 DSH；保留需要审批即拒绝和任务超时取消约束。

## 0.1.31 — 2026-09-05

- 将定时任务编辑器的浮层背景、阴影、主题遮罩和无边框关闭图标与宿主统一，同时保留原有关闭行为。
- 任务总览卡片改为直接打开对应任务设置，不再跳转最近运行会话；同时移除右侧会话箭头、避免计划图标在窄侧栏中被压缩隐藏，并将冗长的英文下次运行文案收紧为右对齐倒计时，同时保留完整无障碍说明。
