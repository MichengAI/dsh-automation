...
- Web 端到端验收发现并修复新版 Connection 独立 RPC 通道缺少 `webServer` 注入导致宿主启动失败的问题；保留 Web Profile 原有的 `webRuntime` 注入，新增真实 Connection 注册与卸载回归。
- 执行记录区分手动、定时和补偿触发。
- 将官方宿主兼容范围限定为 `0.1.0-rc.8 || 0.1.1-rc.2 || 0.1.2-rc.1 || 0.1.5-rc.1`，开发依赖仍锁定 `0.1.5-rc.1`；新增四版本独立安装与执行回归命令 `pnpm test:matrix`。
- 适配 DSH `0.1.5-rc.1` 的显式 Agent 创建回调，保留旧宿主兼容，修复自动化启动失败。
- 兼容新版持久化会话快照，避免启动对账清空仍存在的冷会话关联；缺少持久化枚举时保留关联。
- 升级官方开发依赖和预发布兼容范围，加入不使用运行时桩的 AgentLoop、V3 会话及权限日志回归验证。
