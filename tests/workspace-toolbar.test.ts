...
test('定时会话列表跟随官方文件夹悬停、滚动槽和悬停卡片', () => {
  ...
  assert.match(css, /\.dsh-st-n\.is-017\{--dsh-session-list-scrollbar-width:5px\}/)
  assert.match(css, /\.dsh-st-n\.is-017 \.dsh-st-n-time\{font-size:10px/)
  assert.match(css, /\.dsh-st-n\.is-016 \.dsh-st-n-sess \.dsh-st-n-title\{scroll-behavior:smooth\}/)
  const toolbar = readFileSync(new URL('../src/client/workspace-toolbar.tsx', import.meta.url), 'utf8')
  assert.match(toolbar, /hostHasWorkspaceTree\(\) \? \[\{ id: 'workspace-tree'/)
  assert.match(toolbar, /hostMenuRendersChildren\(\) && onArchivedFilterChange !== undefined/)
