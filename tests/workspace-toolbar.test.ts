...
test('定时会话列表跟随官方文件夹悬停、滚动槽和悬停卡片', () => {
  ...
  assert.match(css, /\.dsh-st-n\{[^}]*--dsh-session-list-scrollbar-width:5px/)
  assert.match(css, /\.dsh-st-n-time\{flex:none;font-size:10px/)
  assert.doesNotMatch(css, /is-017/)
  assert.doesNotMatch(css, /is-016/)
  assert.match(css, /\.dsh-st-n-sess \.dsh-st-n-title\[data-scrolled\]\{mask-image:/)
  const toolbar = readFileSync(new URL('../src/client/workspace-toolbar.tsx', import.meta.url), 'utf8')
  assert.match(toolbar, /\{ id: 'workspace-tree', label: t\('sidebar.groupWorkspaceTree'\)/)
  assert.match(toolbar, /onArchivedFilterChange !== undefined \? \[/)
  assert.doesNotMatch(toolbar, /hostHasWorkspaceTree|hostMenuRendersChildren/)
