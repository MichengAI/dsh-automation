...
test('设置标题旁提供仓库和反馈 GitHub 链接', async () => {
  ...
  assert.match(source, /IconListPenOutline/)
  assert.match(source, /target="_blank"/)
  assert.match(source, /rel="noreferrer"/)
  assert.match(source, /size="small" shape="default"/)
  assert.match(styles, /\.dsh-st-heading-links\{display:inline-flex;align-items:center;gap:8px/)
