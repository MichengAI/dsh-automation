...
test('包保持可安装的 DSH bundle 与 Web client 契约', async () => {
  ...
  assert.equal(manifest.peerDependencies?.['@deepseek-ai/dsh-agent'], '0.1.0-rc.8 || 0.1.1-rc.2 || 0.1.2-rc.1 || 0.1.5-rc.1')
  ...
  assert.equal(manifest.peerDependencies?.['@deepseek-ai/dsh-client-ui-primitives'], '0.1.0-rc.8 || 0.1.1-rc.2 || 0.1.2-rc.1 || 0.1.5-rc.1')
  assert.equal(manifest.peerDependencies?.['@deepseek-ai/dsh-permission-presets'], '0.1.0-rc.8 || 0.1.1-rc.2 || 0.1.2-rc.1 || 0.1.5-rc.1')
  assert.equal(manifest.peerDependencies?.['@deepseek-ai/schemastery'], '>=3.18.1 <4.0.0')
  assert.equal(manifest.devDependencies?.['@deepseek-ai/dsh-agent'], '0.1.5-rc.1')
  assert.equal(manifest.devDependencies?.['@deepseek-ai/dsh-client-ui-primitives'], '0.1.5-rc.1')
  for (const [name, range] of Object.entries(manifest.peerDependencies ?? {})) {
    if (name.startsWith('@deepseek-ai/dsh-')) {
      assert.equal(range, '0.1.0-rc.8 || 0.1.1-rc.2 || 0.1.2-rc.1 || 0.1.5-rc.1', name)
    }
  }
