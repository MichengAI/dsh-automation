...
export type SessionSelector = <Selected>(
  select: (state: SessionListState) => Selected,
  equals?: (left: Selected, right: Selected) => boolean,
) => Selected
export type WorkspaceSelector = <Selected>(
  select: (state: WorkspaceListState) => Selected,
  equals?: (left: Selected, right: Selected) => boolean,
) => Selected
...
export interface NativeSwitcherProps {
  ...
  readonly renderSlot?: (name: string, props?: Record<string, unknown>, opts?: { readonly hookContext?: unknown }) => ReactNode
}
