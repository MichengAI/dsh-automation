...
var HOST_SESSION_RETRY_GAPS = [1, 2, 4];
var HOST_SESSION_MAX_ATTEMPTS = HOST_SESSION_RETRY_GAPS.length + 1;
...
function installAutomationSessionSync(runtime, getSessions) {
  let stopped = false;
  let publication = 0;
  let trackedSessions;
  ...
  const missingSessionKey = (sessions) => {
    ...
    return [...new Set(snapshot.runs.map((run) => run.sessionId).filter((sessionId) => sessionId !== void 0 && sessionId !== ""))].filter((sessionId) => !present.has(sessionId)).sort().join("\0");
  };
  const reconcile = () => {
    if (stopped) return;
    ...
    nextAttemptPublication = retryGap === void 0 ? Number.POSITIVE_INFINITY : publication + retryGap;
...
function createAutomationRuntime(rpc) {
  ...
  const armPoll = () => {
    ...
    }, snapshotPollIntervalMs(state.snapshot?.runs));
  };
  ...
  const publish = (next) => {
    ...
    state = next;
    if (listeners2.size > 0 && previousInterval !== snapshotPollIntervalMs(state.snapshot?.runs)) armPoll();
  ...
  const source = {
    ...
    subscribe: (listener) => {
      ...
      return () => {
        ...
        if (listeners2.size === 0) removePageResumeListeners();
      };
    }
  };
  const refresh = async () => {
    ...
    /* @__PURE__ */ (0, import_jsx_runtime8.jsxs)(
      "button",
      {
        ...
        children: [
          ...
          /* @__PURE__ */ (0, import_jsx_runtime8.jsxs)("span", { className: "dsh-st-overview-next", children: [
            /* @__PURE__ */ (0, import_jsx_runtime8.jsx)("span", { children: t("stats.next") }),
            /* @__PURE__ */ (0, import_jsx_runtime8.jsx)("strong", { children: nextRun })
          ] }),
          row.lastSessionId !== void 0 && /* @__PURE__ */ (0, import_jsx_runtime8.jsx)(ChevronIcon, { width: 12, height: 12, className: "dsh-st-overview-chevron" })
        ]
      }
    ),
...
.dsh-st-overview-open{display:grid;grid-template-columns:minmax(0,1fr) auto 12px;grid-template-rows:18px 18px;align-items:center;gap:4px 8px;width:100%;min-height:56px;padding:8px 9px;border:0;background:transparent;color:inherit;text-align:left;cursor:pointer}
...
.dsh-st-overview-name{box-sizing:border-box;grid-column:1/4;grid-row:1;min-width:0;padding-right:44px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:13px;line-height:18px;font-weight:580}
...
.dsh-st-overview-next{display:inline-flex;grid-column:2;grid-row:2;flex:none;min-width:0;flex-direction:row;align-items:baseline;gap:4px;color:var(--dsw-alias-label-tertiary,#8b8f98);font-size:10px;line-height:14px;white-space:nowrap}
...
.dsh-st-overview-chevron{grid-column:3;grid-row:2;flex:none;color:var(--dsw-alias-label-tertiary,#8b8f98)}
