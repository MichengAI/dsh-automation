...
function installAutomationSessionSync(runtime, sessions) {
  let stopped = false;
  let attemptedMissingKey;
  ...
  const reconcile = () => {
    ...
    hostRefreshPromise = Promise.resolve().then(async () => {
      ...
      if (attemptedMissingKey === missingKey) attemptedMissingKey = void 0;
    }).finally(() => {
